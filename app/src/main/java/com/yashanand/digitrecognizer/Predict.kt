...
class Predict {
    ...
    fun predict(module: Module, resizedBitmap: Bitmap, bool :Int) : ArrayList<Int>{



        Log.d("ResizedBitmap","height"+ resizedBitmap.height.toString() +"width"+ resizedBitmap.width.toString())
        ...
        for (pixelValue in pixels) {
            ...
            if(bool == 0) {
                ...
                pixelArray[i] = normalizedPixelValue
                i += 1
            }else {
        ...
        Log.d("listIndex2:" , inputOfTensor.toString())
        val outputFromTensor = module.forward(IValue.from(inputOfTensor)).toTensor()
        Log.d("listIndex:" , outputFromTensor.toString())
        ...
        val probability = (scores[maxValueIdx]*1000).toInt()
